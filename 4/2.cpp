...
5. 마스터가 되면 오히려 전역변수를 잘 사용할 수 있다.(규칙 파괴)
6. 전역변수는 디버깅 추적이 어렵다. 때문에 g_value 처럼 변수에 명명을 해주는
것도 좋다.
*/
...
// 즉 스태틱은 현재 선언된 스코프 안에서만 유효하다.(물론 포인터를 쓴다면
// 이야기가 달라지겠지만)

// forward declaration mac unsurported
